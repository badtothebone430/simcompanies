...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SimCompanies Cash Flow Statement Analyzer")
        ...
        # KPI cards
        self.kpi_net = KpiCard("Value today")
        self.kpi_income = KpiCard("Value start")
    ...
    def config_path(self) -> str:
        return os.path.join(os.path.dirname(__file__), "config", "sim_cashflow_analyser_config.json")
    ...
    def default_csv_path(self) -> str:
        return os.path.join(os.path.dirname(__file__), "userdata", "cashflow.csv")
    ...
    def normalize_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Custom metric
        if "Total change in cash" not in out.columns:
            if "All income" in out.columns and "All expenses" in out.columns:
                out["Total change in cash"] = out["All income"] + out["All expenses"]

        # Reorder columns: Timestamp, Date, then rest
        cols = ["Timestamp", "Date"]
        rest = [c for c in out.columns if c not in cols]
        if "Total change in cash" in rest:
            rest.remove("Total change in cash")
            rest.insert(0, "Total change in cash")
        cols.extend(rest)
    ...
    def populate_metric_combo(self, df: pd.DataFrame):
        ...
        if "Total change in cash" in numeric_cols:
            numeric_cols = ["Total change in cash"] + [c for c in numeric_cols if c != "Total change in cash"]
        self.metric_combo.addItems(numeric_cols)
        if "Total change in cash" in numeric_cols:
            self.metric_combo.setCurrentText("Total change in cash")
        elif "All income" in numeric_cols:
            self.metric_combo.setCurrentText("All income")
        elif numeric_cols:
    ...
    def current_metric(self, view: pd.DataFrame) -> str:
        ...
        if "Total change in cash" in numeric_cols:
            return "Total change in cash"
        if "All income" in numeric_cols:
            return "All income"
        return numeric_cols[0] if numeric_cols else "Total change in cash"
    ...
    def update_summaries(self):
        ...
        if not daily.empty and metric in daily.columns:
            balance_start = float(daily.iloc[0][metric])
            balance_today = float(daily.iloc[-1][metric])
