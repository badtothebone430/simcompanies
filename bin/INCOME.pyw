...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SimCompanies Income Statement Analyzer")
        ...
        # KPI cards
        self.kpi_net = KpiCard("Net income")
        self.kpi_income = KpiCard("Total income")
        self.kpi_expense = KpiCard("Total expenses")
    ...
    def config_path(self) -> str:
        return os.path.join(os.path.dirname(__file__), "config", "sim_income_analyser_config.json")
    ...
    def default_csv_path(self) -> str:
        return os.path.join(os.path.dirname(__file__), "userdata", "income.csv")
    ...
    def normalize_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Gross Profit" not in out.columns:
            gross_profit_cols = ["Sales", "COGS"]
            if any(c in out.columns for c in gross_profit_cols):
                out["Gross Profit"] = sum_columns(gross_profit_cols)

        if "Operating Expenses" not in out.columns:
            operating_cols = [
                "Construction",
                "Exchange fees",
                "Executives costs",
            ...
            if any(c in out.columns for c in operating_cols):
                out["Operating Expenses"] = sum_columns(operating_cols)

        if "Total Comprehensive Income" not in out.columns:
            tci_cols = ["NetIncome", "Other Comprehensive Income"]
            if any(c in out.columns for c in tci_cols):
                out["Total Comprehensive Income"] = sum_columns(tci_cols)

        # Reorder columns: Timestamp, Date, then rest (NetIncome last)
        cols = ["Timestamp", "Date"]
        rest = [c for c in out.columns if c not in cols]
        if "NetIncome" in rest:
            rest.remove("NetIncome")
        for metric in ("Gross Profit", "Operating Expenses", "Total Comprehensive Income"):
            if metric in rest:
                rest.remove(metric)
                rest.append(metric)
        if "NetIncome" in out.columns:
            rest.append("NetIncome")
        cols.extend(rest)
    ...
    def populate_metric_combo(self, df: pd.DataFrame):
        ...
        if "Total Comprehensive Income" in numeric_cols:
            numeric_cols = ["Total Comprehensive Income"] + [c for c in numeric_cols if c != "Total Comprehensive Income"]
        self.metric_combo.addItems(numeric_cols)
        if "Total Comprehensive Income" in numeric_cols:
            self.metric_combo.setCurrentText("Total Comprehensive Income")
        elif "NetIncome" in numeric_cols:
            self.metric_combo.setCurrentText("NetIncome")
    ...
    def refresh_chart(self):
        metric = self.metric_combo.currentText() or "Total Comprehensive Income"
    ...
    def update_summaries(self):
        ...
        if view is None or view.empty:
            ...
            self.kpi_count.value_lbl.setText("0")
        ...
        net_total = float(view["NetIncome"].sum()) if "NetIncome" in view.columns else 0.0
        income_total = 0.0
        expense_total = 0.0
        for col in numeric_cols:
            if col == "NetIncome":
                continue
            vals = pd.to_numeric(view[col], errors="coerce").fillna(0.0)
            income_total += float(vals[vals > 0].sum())
            expense_total += float(vals[vals < 0].sum())
        expense_total = -expense_total
        cnt = int(len(view))

        self.kpi_net.value_lbl.setText(f"{net_total:,.2f}")
        self.kpi_income.value_lbl.setText(f"{income_total:,.2f}")
        self.kpi_expense.value_lbl.setText(f"{expense_total:,.2f}")
        self.kpi_count.value_lbl.setText(f"{cnt:,}")
        ...
        if "Date" in view.columns and metric in view.columns:
            ...
            daily = pd.DataFrame(columns=["Date", metric])
    ...
    def update_projection(self):
        ...
        avg = float(pd.to_numeric(daily[metric], errors="coerce").fillna(0).mean())
